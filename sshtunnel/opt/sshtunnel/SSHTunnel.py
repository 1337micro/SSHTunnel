...
global LOGNAME
LOGNAME = getpass.getuser() #Gets the current username
...
class StartingWindow(QDialog):
    ...
    def __init__(self, parent= None):
        ...
        self.bExistingProxy = QPushButton("I Have a Server or VPS")
...
class LaunchApp(QDialog):
    ...
    def __init__(self, parent=None):
        ...
        self.info_browse = QLabel("""
                            <b> IF YOU ARE NOT USING A KEY FILE: PAY ATTENTION TO THE TERMINAL, YOU MUST ENTER YOUR PASSWORD.</b><br/>
                            <b> If your remote shell does not appear soon, you must close this window and try to reconnect </b> <br/><br/>
                           <b>SOCKS 5 proxy:</b> routed locally (<em>ip: 127.0.0.1</em>) on the port: <em>1080</em> <br/>
    ...
    def launch_program(self):
        ...
        #General terminals that support executing a program with the -e option
        #In cases where different flavours of the terminal exist, only the most basic case is added
        #(i.e. mrxvt and xvt (only xvt is added))
        list_e = ["xterm","aterm","stterm","xvt","rxvt","vala-terminal",
          "terminator","pterm","evilvte","lxterminal","mlterm","termit","xiterm+thai"]
        #General terminals that support executing a program with the -x option        
        list_x = ["sakura","gnome-terminal","xfce4-terminal"] 
        
        pref_term = self.preffered_terminal() # The preffered terminal
        
        print (pref_term)
        
        if pref_term in list_e:
            subprocess.Popen(pref_term + " -e tsocks " + args, shell=True, universal_newlines = True) # MUST USE shell=True otherwise user wont be able to enter information
        elif pref_term in list_x:
            subprocess.Popen(pref_term + " -x tsocks " + args, shell=True, universal_newlines = True)
        else:
            #preffered COLORTERM does not support launching an application with -e or -x
            #We will launch with xterm
           subprocess.Popen("xterm -e tsocks " + args, shell=True, universal_newlines = True)
    
    def preffered_terminal(self):
        """Returns a string containing the name of the user's preffered COLORTERM terminal from the current 'env',
        otherwise, returns the preffered TERM from the current 'env' (usually xterm)  
        """
...
# Enter Qt application main loop (show the application)
app.exec_()
